 ...
 * provider for a 'create user' scenario.
 *
 ...
 *
 * This file focuses on the differences required to test the creation of a new
 * user, specifically when the provider returns a 201 Created with a JSON body
 * containing the new user's details. It is intended as a reference for users
 * who want to extend their contract tests to cover resource creation scenarios.
 *
 * Key differences from `get_user.c`:
 *
 * - The HTTP method is POST and the endpoint is /users.
 * - The request body is a JSON object with the new user's name: {"name":
 *   "Bob"}.
 * - The expected response status is 201 (Created) instead of 200.
 * - The response body is a JSON object with id, name, and created_on fields,
 *   using Pact matchers.
...
static InteractionHandle create_and_configure_interaction(PactHandle pact) {
  ...
  InteractionHandle interaction =
      pactffi_new_interaction(pact, "A request to create a new user");
  pactffi_upon_receiving(interaction, "A request to create a new user");
  pactffi_with_request(interaction, "POST", "/users");
  pactffi_with_header_v2(interaction, InteractionPart_Request, "Content-Type",
                         0, "application/json");
  // Request body: {"name": "Bob"}
  pactffi_with_body(interaction, InteractionPart_Request, "application/json",
                    "{\"name\": \"Bob\"}");
