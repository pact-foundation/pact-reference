 ...
 * @brief Example of using Pact FFI in C to test a consumer against a mock provider for a 'user not found' scenario.
 *
 ...
 *   - The response body is a JSON object with an error message: {"detail": "User not found"}.
 *   - The test asserts that the client receives a 404 and the correct error message.
 *
...
static InteractionHandle create_and_configure_interaction(PactHandle pact)
{
    ...
    InteractionHandle interaction = pactffi_new_interaction(pact, "A request for an unknown user");
