/**
 * @file delete_user.c
 * @brief Example of using Pact FFI in C to test a consumer against a mock provider for a 'delete user' scenario.
 *
 ...
 * specifically when the provider returns a 204 No Content. It is intended as a reference for users who want to extend their
 * contract tests to cover resource deletion scenarios.
 *
 ...
 * the DELETE /users/124 endpoint. The provider state is set to indicate the user exists,
 * the request is a DELETE, and the response status is 204.
 *
...
static InteractionHandle create_and_configure_interaction(PactHandle pact)
{
    ...
    InteractionHandle interaction = pactffi_new_interaction(pact, "A user deletion request");
 ...
 * uses cURL to perform the DELETE request. It asserts that the response is a 204.
 * See get_user.c for the general structure.
 *
