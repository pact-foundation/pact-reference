...
/**
 * @file get_user.c
 * @brief Example of using Pact FFI in C to test a consumer against a mock provider.
 *
...
static InteractionHandle create_and_configure_interaction(PactHandle pact)
{
    ...
    InteractionHandle interaction = pactffi_new_interaction(pact, "A user request");
 ...
 *
 * This function checks if the mock server interactions matched the expectations.
 * If there are mismatches, it prints the details and fails the test. It also writes
 * the Pact file to disk for provider verification and cleans up the mock server and
 * Pact model resources.
 *
