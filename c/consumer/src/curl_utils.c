...
{
    log_debug("response_buffer_callback called with size=%zu, nmemb=%zu", size, nmemb);
...
static int curl_perform_common(const char *url, const char *method, const char *body, struct curl_slist *headers, struct ResponseBuffer *response)
{
    log_debug("curl_perform_common called with url=%s, method=%s", url, method ? method : "GET");
