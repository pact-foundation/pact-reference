...
def prevTag = 'git describe --abbrev=0  --tags --match=libpact_models-* --first-parent'.execute().text.trim()
def changelog = []
...
ask('Tag and Push commits?: [Y]') {
  ...
  executeOnShell("git tag libpact_models-v${releaseVer}")
...
ask("Bump version to $nextVer?: [Y]") {
  ...
  executeOnShell "sed -i -e 's/documentation = \"https:\\/\\/docs\\.rs\\/pact_models\\/${releaseVer}\\/pact_models\\/\"/documentation = \"https:\\/\\/docs\\.rs\\/pact_models\\/${nextVer}\\/pact_models\\/\"/' Cargo.toml"
