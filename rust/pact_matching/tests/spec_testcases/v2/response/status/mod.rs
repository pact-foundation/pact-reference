...
async fn matches() {
    println!("FILE: tests/spec_testcases/v2/response/status/matches.json");
    #[allow(unused_mut)]
    ...
    let expected = http_interaction_from_json("tests/spec_testcases/v2/response/status/matches.json", &interaction_json, &PactSpecification::V2).unwrap();
    ...
    let actual = http_interaction_from_json("tests/spec_testcases/v2/response/status/matches.json", &interaction_json, &PactSpecification::V2).unwrap();
...
async fn different_status() {
    println!("FILE: tests/spec_testcases/v2/response/status/different status.json");
    #[allow(unused_mut)]
    let mut pact: serde_json::Value = serde_json::from_str(r#"
      {
        "match": false,
    ...
    let interaction_json = serde_json::json!({"type": "Synchronous/HTTP", "response": pact.get("expected").unwrap()});
    let expected = http_interaction_from_json("tests/spec_testcases/v2/response/status/different status.json", &interaction_json, &PactSpecification::V2).unwrap();
    println!("EXPECTED: {:?}", expected);
    println!("BODY: {}", expected.as_request_response().unwrap().response.body.display_string());
    let interaction_json = serde_json::json!({"type": "Synchronous/HTTP", "response": pact.get("actual").unwrap()});
    let actual = http_interaction_from_json("tests/spec_testcases/v2/response/status/different status.json", &interaction_json, &PactSpecification::V2).unwrap();
    println!("ACTUAL: {:?}", actual);
    println!("BODY: {}", actual.as_request_response().unwrap().response.body.display_string());
    let pact_match = pact.get("match").unwrap();
